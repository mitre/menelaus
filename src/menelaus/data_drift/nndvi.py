...
class NNDVI(DriftDetector):
    ...
    def update(self, test_batch: np.array):
        ...
        theta_drift = self._compute_drift_threshold(M_nnps, v_ref, v_test, self.sampling_times, self.alpha)
    ...
    def _compute_drift_threshold(M_nnps, v_ref, v_test, sampling_times, alpha):
        ...
        for _ in range(sampling_times):
            ...
            v2_shuffle = np.random.permutation(v_test)
            d_i_shuffle = NNSpacePartitioner.compute_nnps_distance(M_nnps, v1_shuffle, v2_shuffle)
