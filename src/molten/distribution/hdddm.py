...
class HDDDM(DriftDetector):
    ...
    def update(self, test_batch):
        ...
        # Hellinger distances
        self.current_distance, feature_distances = self.hellinger_distance(self.reference_density, test_density)
        self.distances[self.total_samples]= self.current_distance

        # Difference in distances for each feature 
        if self.total_samples > 1:
            self.feature_epsilons = [a_i - b_i for a_i, b_i in zip(feature_distances, self.prev_feature_distances)] 
        ...
        if self.samples_since_reset > 1:
            ...
            if current_epsilon > self.beta:
                ...
                self.feature_info = {
                    ...
                    'largest drift in variable ': self.feature_epsilons.index(max(self.feature_epsilons)),
                                     }
    ...
    def build_histograms(self, dataset, min_values, max_values):
        ...
        histograms = [np.histogram(dataset.iloc[:, f], bins=self.bins, range=(min_values[f], max_values[f]))[0] for f in range(self.d)]
    ...
    def hellinger_distance(self, reference_density, test_density):
        ...
        for f in range(self.d):
            ...
            for b in range(self.bins):
                f_distance += (np.sqrt(t_density[b]/t_length) - np.sqrt(
                    r_density[b]/r_length)) ** 2
    ...
    def adaptive_threshold(self, stat, test_n):
        """Computes adaptive threshold. If computing threshold for third test batch, removes our
         estimate of epsilon_0 from future estimates of epsilon_hat and std
        ...
        total_stdev = sum((self.epsilon[i] - epsilon_hat)**2 for i in range(len(self.epsilon) - 1))
        stdev = np.sqrt(total_stdev / (d_scale) )

        if stat == "tstat":
            t_stat = scipy.stats.t.ppf(1-(self.significance/2), self.reference_n + test_n - 2)
    ...
    def estimate_initial_epsilon(self, reference, num_subsets, histogram_mins, histogram_maxes):
        ...
        for train, test in kf.split(reference):
            bootstraps.append(self.build_histograms(reference.iloc[test,:], histogram_mins, histogram_maxes))
        ...
        for df_indx in range(len(bootstraps)):
            ...
            while j < len(bootstraps):
                distances.append(self.hellinger_distance(bootstraps[df_indx], bootstraps[j])[0]) 
                j += 1 
