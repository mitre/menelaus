class DriftDetector():
    ...
    Attributes:
    :attribute drift_state The detector's status as of its last update. Has a value of 'drift', 'warning', or None.
        Only some detectors have a 'warning' state.
    """
    def __init__(self, *args, **kwargs):
        super().__init__()
        self.total_samples = 0  # number of samples the drift detector has ever been updated with
    ...
    def reset(self, *args, **kwargs):
        ...
        drift_state == 'drift'.
        """
        self.n = 0  # number of elements the detector has been updated with since last reset
    ...
    def drift_state(self, value):
        if not value in ('drift', 'warning', None):
            raise ValueError('DriftDetector._drift_state must be "drift", "warning", or None.')
        else:
