...
class DL_Detector(BatchDetector):
    ...
    def reset(self):
        ...
        if self.total_batches == 0 and self.scaling == True:
            ...
            self.reference =  pd.DataFrame(scaler.fit_transform(self.reference), columns = self._input_cols, index = self.ref_idx)
    ...
    def update(self, X, by_feature=True, X_by_feature=None, y_true=None, y_pred=None):
        ...
        if self.scaling == True: 
            scaler = StandardScaler()
            X =  pd.DataFrame(scaler.fit_transform(X), columns = self._input_cols, index = X_idx)
        ...
        if by_feature == True:
            ...
            for f in self._input_cols:
                ...
                X_joined, y_joined = self._join_data(pd.DataFrame(self.reference.loc[:,f]), pd.DataFrame(X.loc[:,f]))
                pred = self._obtain_proba(X_joined, y_joined)
                test_statistic, pvalue = self._ks_test(pred)  # TODO consider a generic divergence test?
                ...
                self.feature_statistics[f] = {'Statistic': test_statistic, 
                                     'Pvalue': pvalue, 
                                     'Drift': f_drift_state}
            ...
            self.drift_statistics = {'Statistic': test_statistic, 
                                     'Pvalue': pvalue, 
    ...
    def _obtain_proba(self, X_joined, y_joined):
        ...
        for i,(train_index, test_index) in enumerate(CV.split(X_joined,y_joined)):
            ...
            pred_i = pd.DataFrame({'y_true': y_joined.iloc[test_index], 
