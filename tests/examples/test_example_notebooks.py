...
Taken from github.com/AureumChaos/LEAP/
"""
import glob
import os
import pathlib
...
def run_notebook(path, timeout=120):
    ...
    c.ExecutePreprocessor.kernel_name = (
        "venv"  # We assume a kernel named "venv" that lives in our venv
    )
...
def is_hidden_path(path):
    """Return true if the file or any of its ancestor directories begins with '.'."""
...
# We give Jupyter tests a separate marker, because they can only run if the 'venv' kernel is configured propertly by the user
@pytest.mark.jupyter
