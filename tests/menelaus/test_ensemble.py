...
def test_batch_ensemble_1():
    ...
    be = BatchEnsemble(detectors={'k1': kdq1, 'k2': kdq2, 'k3': kdq3}, evaluator='simple-majority')
...
def test_batch_ensemble_2():
    ...
    be.set_reference(df.loc[:0])
    be.update(df.loc[1:])
    assert len(be.detectors['k1'].input_cols) == 1
    assert len(be.detectors['k2'].input_cols) == 2
