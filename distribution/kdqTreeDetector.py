...
class kdqTreeDetector(DriftDetector):
    ...
    use kdq-trees.
    """
    def __init__(self, window_size, min_points_in_bin=100, num_bootstrap_samples=500, gamma=0.05, alpha=0.01, verbose=True):
        """
    ...
    def update(self, next_obs):
        ...
        if self._build_reference_and_test:
            if len(self.window_data['reference']) < self._window_size:
                self.window_data['reference'] = self.window_data['reference'].append(next_obs)
                ...
                if len(self.window_data['test']) == self._window_size:
                    ...
                    self._kdqTreeNodes = self._kdqTreeBuildNodes(data_input=self.window_data['reference'],
                    ...
                    self.window_data['reference'].index.name = self.window_data['test'].index.name
                    ...
                    for i in range(len(self.window_data['test'])):
                        test_bins.append(
                            self._kdq_tree_binner(kdqTreeNodes=self._kdqTreeNodes,
                                          data_point=self.window_data['test'].iloc[[i]]))
                    self.window_data['test'] = self.window_data['test'].assign(bin=test_bins)

                    # Build bootstrap samples from the reference window
                    bootstrap_samples = self._bootstrapping(self.window_data['reference']['bin'].tolist(),
                                                      2 * self._window_size, self._num_bootstrap_samples)
                    bootstrap_types = [
                        [self._compute_type(x[:len(x) // 2], self._alphabet),
                         self._compute_type(list(x[(len(x) // 2) + 1:]), self._alphabet)]
                        for x in bootstrap_samples]
                    bootstrap_KL_distances = [kl_divergence(x[0], x[1]) for x in bootstrap_types]
                    bootstrap_KL_distances.sort()

                    self._critical_distance = bootstrap_KL_distances[int(np.ceil(self._num_bootstrap_samples * self._alpha))]

        else:
            next_obs = next_obs.assign(bin=self._kdq_tree_binner(kdqTreeNodes=self._kdqTreeNodes, data_point=next_obs))

            self.window_data['test'] = self.window_data['test'].iloc[1:, :].append(next_obs)

            type1 = self._compute_type(self.window_data['reference']["bin"].tolist(), self._alphabet)
            type2 = self._compute_type(self.window_data['test']["bin"].tolist(), self._alphabet)
            ...
            if dist > self._critical_distance:
                ...
                if self._c > self._gamma*self._window_size:
                    ...
                    kdqbins_filtered = self._kdq_tree_build_bins(self._kdqTreeNodes[self._kdqTreeNodes['depth'] < 9])
                    ...
                    self.drift_location["spatial_scan_statistic"].append(
                        self._kulldorff_spatial_scan_statistic(
                            ...
                            bins_w1=[x for x, y in zip(list(kdqbins_filtered.bin), list(kdqbins_filtered.id))
                                     if y in self.window_data['reference'].index.tolist()],
                            bins_w2=[x for x, y in zip(list(kdqbins_filtered.bin), list(kdqbins_filtered.id))
                                     if y in self.window_data['test'].index.tolist()]).sort_values(
                    ...
                    self._build_reference_and_test = True
                    self.window_data['reference'] = self.window_data['test'].drop(columns=['bin'])
    ...
    def drift_visualization(self, id_date_df=None, save_fig=None):
        if id_date_df is None:
            kl_distance_ts = pd.DataFrame(self.drift_tracker, index=range(len(self.drift_tracker["dist"])))

            with plt.style.context('fivethirtyeight'):
                plt.figure(figsize=(20, 8))
                plt.plot(kl_distance_ts['id'], kl_distance_ts["dist"], linestyle='-', marker=',', zorder=0,
                         linewidth=.85)
                plt.plot(kl_distance_ts['id'], kl_distance_ts["critical_distance"], linestyle='--', marker=',',
                         linewidth=1, color="goldenrod", zorder=1)
                plt.vlines(x=kl_distance_ts.loc[kl_distance_ts.dist >= kl_distance_ts.critical_distance, :].id,
                           ymin=min(kl_distance_ts["dist"]), ymax=max(kl_distance_ts["critical_distance"]),
                           linestyle='-', color="darkred", alpha=0.05, linewidth=.5, zorder=2)
            ...
            kl_distance_ts = pd.DataFrame(self.drift_tracker, index=range(len(self.drift_tracker["dist"]))).merge(id_date_df,
                                                                                                          how="left",
                                                                                                          on="id")
            with plt.style.context('fivethirtyeight'):
                plt.figure(figsize=(20, 8))
                plt.plot_date(kl_distance_ts.date, kl_distance_ts["dist"], linestyle='-', marker=',', zorder=0,
                              linewidth=.85)
                plt.plot_date(kl_distance_ts.date, kl_distance_ts["critical_distance"], linestyle='--', marker=',',
                              linewidth=1, color="goldenrod", zorder=1)
                plt.vlines(x=kl_distance_ts.loc[kl_distance_ts.dist >= kl_distance_ts.critical_distance, :].date,
                           ymin=min(kl_distance_ts["dist"]), ymax=max(kl_distance_ts["critical_distance"]),
                           linestyle='-', color="darkred", alpha=0.05, linewidth=.5, zorder=2)
                plt.title("Kullback-Leibler Divergence", fontsize=16, y=1.01)
                plt.suptitle("KDQ Tree Drift Detection", fontsize=24, y=0.97)
    ...
    def drift_location_visualization(self):
        ...
        for t in range(len(self.drift_location["spatial_scan_statistic"])):
            ...
            bin_df = pd.DataFrame(
                columns=['kdqTree'] + ["split" + str(x) for x in range(11)[1:]] + ['spatial_scan_statistic'])
            bins = spatial_scan_statistic['bin'].tolist()

            for j in range(len(bins)):

                tmp_bin = bins[j]
                tmp_statistics = spatial_scan_statistic['kulldorff_spatial_scan_statistic'].tolist()[j]
            ...
            figs.append(
                px.treemap(bin_df, path=['kdqTree'] + ["split" + str(x) for x in range(bin_df.shape[1] - 2)[1:]],
                           color='spatial_scan_statistic', color_continuous_scale="blues",
    ...
    def _bootstrapping(self, list_input: list, sample_size: int, num_samples: int):
        ...
        for i in range(num_samples):
            bootstrap_samples.append(np.random.choice(list_input, size=sample_size).tolist())
    ...
    def _compute_type(self, multiset: list, alphabet: list):
        ...
        type = [(multiset.count(a) + 0.5) / (len(multiset) + len(alphabet) / 2) for a in alphabet]

        return type

    def _kdq_tree_build_bins(self, clean_results):
        """
        Builds kdqTree bins from nodes and splits defined in the output of kdqTreeBuildNodes()
        :param: clean_results (DataFrame): Dataframe of nodes and splits produced from kdqTreeBuildNodes()
        ...
        for col in clean_results.columns[7:]:
            try:
                tmp_df = clean_results.loc[clean_results[col] == True, :][['depth', 'bin_id', col]].sort_values(
                    by='depth',
                    ascending=False)
            except:
                tmp_df = clean_results.loc[clean_results[col].iloc[:, 0] == True, :][
                    ['depth', 'bin_id', col]].sort_values(
                    by='depth',
                    ascending=False)
            ids.append(col[1:])
            bins.append(tmp_df.bin_id.tolist()[0])

        kdqbins = pd.DataFrame({"id": [int(x) for x in ids], "bin": bins}).sort_values(by='bin')
    ...
    def _kdq_tree_splits(self, data_input: pd.DataFrame, original_ids: list, node: dict, node_id: int, depth: int, axis: int,
                      min_points: int, max_value: int, min_value: int, verbose=True):
        """
        ...
        if data_input.shape[0] > min_points:
            ...
            bools_df = pd.DataFrame([(x in list(data_input.index)) for x in original_ids]).transpose()
            ...
            node['results'] = pd.concat(
                [pd.DataFrame({"node_id": node_id, "depth": depth, "axis": list(data_input.columns)[axis],
                               "num_points": data_input.shape[0], "cutpoint": cutpoint, "type": node_type}, index=[0]),
                 bools_df], axis=1)
            ...
            node['left_child'] = self._kdq_tree_splits(data_input.loc[left_of_cutpoint, :], original_ids=original_ids,
                                               node=node.copy(), node_id=2 * node_id, depth=depth, axis=axis_next,
                                               max_value=max_next, min_value=min_next, min_points=min_points,
                                               verbose=verbose)

            node['right_child'] = self._kdq_tree_splits(data_input.loc[[not x for x in left_of_cutpoint], :],
                                                original_ids=original_ids,
                                                node=node.copy(), node_id=(2 * node_id) + 1, depth=depth,
                                                axis=axis_next,
                                                max_value=max_next, min_value=min_next, min_points=min_points,
                                                verbose=verbose)
    ...
    def _kdq_tree_binner(self, data_input: pd.DataFrame, min_points=200, verbose=True):
        """
        ...
        results = pd.concat([pd.DataFrame({"node_id": 1, "depth": 0, "axis": None, "num_points": data_input.shape[0],
                                           "cutpoint": None, "type": "Node"}, index=[0]), bools_df], axis=1)
        ...
        nested_dict_results = self._kdq_tree_splits(data_input=data_input, original_ids=original_ids,
                                            node=node, node_id=1, depth=depth, axis=1,
        ...
        clean_results = clean_results.sort_values("node_id").assign(bin_id=range(clean_results.shape[0]))
        clean_results = clean_results[clean_results.columns.tolist()[-1:] + clean_results.columns.tolist()[:-1]]
    ...
    def _kulldorff_spatial_scan_statistic(self, alphabet, bins_w1, bins_w2):
        ...
        statistic = [
            ...
            ((len_w1 - w1_counts + 0.5) * np.log((len_w1 - w1_counts + 0.5) / (len_w2 - w2_counts + 0.5))) -
            ((len_w1 + 1) * np.log((len_w1 + 1) / (len_w2 + 1))) for
            w1_counts, w2_counts, in zip(w1_in_alpha, w2_in_alpha)
        ]

        return pd.DataFrame({"bin": alphabet, "kulldorff_spatial_scan_statistic": statistic})
    ...
    def _tree_parser(self, kdqTreeNodes, bin_id):
        ...
        for id in node_ids[::-1]:
            ...
            split_string = split_string + f"{tmp_row['axis'].tolist()[0]} {operator} {round(tmp_row['cutpoint'].tolist()[0],2)}, "
